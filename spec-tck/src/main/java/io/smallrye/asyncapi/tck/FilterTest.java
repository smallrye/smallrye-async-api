...
public class FilterTest extends AppTestBase {
    ...
    public static WebArchive createDeployment() {
        return ShrinkWrap.create(WebArchive.class, "filter.war")
                .addPackages(true, "io.smallrye.asyncapi.apps.filter")
                .addAsManifestResource("gitter.json", "asyncapi.json")
                .addAsManifestResource("filter.properties", "microprofile-config.properties");
    }
    ...
    public void testInfo(String type) {
        ValidatableResponse vr = callEndpoint(type);
        vr.body("info.title", equalTo("Filter API"));
